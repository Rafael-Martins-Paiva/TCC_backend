...
from domain.accounts.exceptions.auth_exceptions import InvalidVerificationTokenError, UserAlreadyExistsError, InvalidOldPasswordError
...
class AuthFlowsAPITest(APITestCase):
    ...
    def test_email_verification_success(self, MockEmailVerificationService):
        mock_service = MockEmailVerificationService.return_value
    ...
    @patch('accounts.views.EmailVerificationService')
    def test_email_verification_invalid_token(self, MockEmailVerificationService):
        mock_service = MockEmailVerificationService.return_value
    ...
    def test_resend_verification_email_success(self, mock_dispatch, MockEmailVerificationService):
        mock_service = MockEmailVerificationService.return_value
        ...
        def side_effect_func(*args, **kwargs):
            mock_dispatch(UserRegistered(user_id=mock_user_entity.id, email=str(mock_user_entity.email), verification_token=mock_user_entity.verification_token))
        ...
        mock_dispatch.assert_called_once_with(UserRegistered(user_id=mock_user_entity.id, email=str(mock_user_entity.email), verification_token=mock_user_entity.verification_token))

    @patch('accounts.views.EmailVerificationService')
    def test_resend_verification_email_not_found(self, MockEmailVerificationService):
        mock_service = MockEmailVerificationService.return_value
    ...
    def test_change_password_success(self, MockChangePasswordService):
        self.client.force_authenticate(user=self.user)
        mock_service = MockChangePasswordService.return_value
    ...
    @patch('accounts.views.ChangePasswordApplicationService')
    def test_change_password_invalid_old_password(self, MockChangePasswordService):
        self.client.force_authenticate(user=self.user)
        mock_service = MockChangePasswordService.return_value
